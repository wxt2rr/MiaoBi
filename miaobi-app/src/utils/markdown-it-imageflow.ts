...
const imageFlowPlugin = (md: MarkdownIt, opt?: any) => {
  ...
  const tokenize = (state: any, start: number) => {
    let token;
  ...
  (md.renderer as any).rules.imageFlow = (tokens: any[], idx: number) => {
  ...
  (md as any).block.ruler.before("paragraph", "imageFlow", tokenize);
};
